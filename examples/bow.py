...
class TorchBOWClassifier(torch.nn.Module):
    def __init__(self, vocab_size: int, num_classes: int) -> None:
        super().__init__()
        self.l1 = torch.nn.Linear(vocab_size, 64)
...
def main():
    # 1000 Samples
    # train_data = load_dataset("imdb", split="train[:10000]").shuffle()
        ...
        for i, sample in pbar:
            ...
            if i % batch_size == 0 and i != 0:
                ...
                if TORCH:
                    ...
                    print(pred_idxs.numpy(), target_idxs.numpy(), batch_correct)
                    ...
                    print(pred_idxs, target_idxs, batch_correct)
                    print(total_correct, total_samples)
                    accuracy = total_correct / total_samples
                    loss = loss.data[0]
